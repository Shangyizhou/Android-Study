...
public class HttpLoggingInterceptor implements Interceptor {
    private static final String LOG_TAG = "HttpLogInterceptor";
    private static final Charset UTF8 = Charset.forName("UTF-8");
    private static final int LOGGABLE_MAX_RESPONSE_CONTENT_LENGTH = 512 * 1024;

    private static void log(String msgText) {
        SLog.d(LOG_TAG, msgText); // TODO 这会暴露很多信息, 后期需要改成VERBOSE级别(DuLog.v)或DEBUG级别(DuLog.d)
    }
    ...
    public Response intercept(Chain chain) throws IOException {
        log("--------HttpLoggingInterceptor intercept ---");
...
//                log(name + ": " + headers.value(i));
                ...
                if (!TextUtils.isEmpty(name) && !"Content-Type".equalsIgnoreCase(name) && !"Content-Length".equalsIgnoreCase(name)) {
                    log(name + ": " + headers.value(i));
                }
            }
        ...
        if (logHeaders) {
            ...
            if (!logBody || !hasBody(response)) {
                ...
                if (contentLength > 0 && contentLength < LOGGABLE_MAX_RESPONSE_CONTENT_LENGTH) {
                    log("");
    ...
    private static long contentLength(Response response) {
        ...
        final String s = headers.get("Content-Length");
        if (s == null) {
            return -1;
        }
        try {
